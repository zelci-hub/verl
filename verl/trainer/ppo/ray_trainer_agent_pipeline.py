...
class RayPPOPipelineAgentTrainer(RayPPOAgentTrainer):
                ...
                with Timer('step', timing_raw):
                    ...
                    def create_replay_queue(generator, q):
                        with Timer('gen', timing_raw):
                            for gen_idx, trajectory, traj_idx, env in enumerate(generator):
                                if trajectory is None:
                                    return
                                q.put((batch_iter, gen_idx, trajectory, traj_idx, env))     
                    # Get the generator function which will yield results as they complete
                    gen_seq_generator = self.generate_agent_trajectories_async(env, timing_raw=timing_raw, original_batch=batch)
                    ...
                    for mini_batch_iter in range(num_loops):
                        ...
                        with Timer('pipeline_gen', timing_raw):
                            ...
                            for _ in range(ppo_mini_batch_size):
                                _, _, traj, traj_idx, env = replay_queue.get()
                                trajectories.append(traj)
                                trajectory_idxs.append(traj_idx)
                                envs.append(env)
                            mini_batch = self._transform_agent_trajectories_helper(envs=envs, trajectory_idxs=trajectory_idxs, trajectories=trajectories)
    ...
    def _validate_agent(self):
        ...
        for test_data in self.val_dataloader:
            ...
            trajectory_idxs = []
            envs = []
            for traj, traj_idx, env in test_traj_generator: 
                trajectories.append(traj)
                trajectory_idxs.append(traj_idx)
                envs.append(env)
            test_output_gen_batch = self._transform_agent_trajectories_helper(envs=envs, trajectory_idxs=trajectory_idxs, trajectories=trajectories)
    ...
    def generate_agent_trajectories_async(self, env, timing_raw={}, original_batch=None):
        self.agent.update_env(env)

        trajectories_generator = self.agent.interact_environment_generator(timing_raw=timing_raw, original_batch=original_batch)

        try:
            for trajectory, traj_idx in trajectories_generator:
                yield trajectory, traj_idx, env
        finally:
