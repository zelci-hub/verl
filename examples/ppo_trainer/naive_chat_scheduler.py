...
class OpenAICompletionScheduler(ChatCompletionScheduler):
    """
    Implementation of ChatCompletionScheduler that uses OpenAI's Completion API
    instead of the ChatCompletion API.
    """
    ...
    async def generate_sequences(self, batch: DataProto, **sampling_params) -> DataProto:
        ...
        async def callback(completions: Completion, info: Dict[str, Any], exception: Exception):
            completion_prompt, batch_completions, batch_index = (
                info["completion_prompt"],
                info["batch_completions"],
                info["batch_index"],
            )
            ...
            comps = []
            for choice in completions.choices:
                comps.append(choice.text)
            batch_completions[batch_index] = comps

        # TODO: we may need to control max concurrent requests here, or it will harm prefix cache hit rate.
        tasks, batch_completions = [], [None] * len(batch)
                ...
                asyncio.create_task(
                    self.submit_completions(  # Changed from submit_chat_completions
                        ...
                        callback_additional_info={
                            ...
                            "batch_index": batch_index,
                            "completion_prompt": formatted_prompt,
                        },
                        model=self.model_name,
        ...
        return self._postprocess(batch, batch_completions, kwargs["n"])
    ...
    def _postprocess(self, batch: DataProto, batch_completions: List[List[str]], n: int) -> DataProto:
        ...
        # prompts: [prompt] from input dataset
        prompts = batch.non_tensor_batch["formatted_prompts"]

        # flatten batch_completions if n > 1
        assert len(batch_completions) == len(prompts)
        batch_completions = [completion for completions in batch_completions for completion in completions]
        assert len(batch_completions) == len(prompts) * n

        # For Completion API, we don't need to apply chat template - responses are already text
        # sequences: [prompt + response]
        sequences = [prompts[i // n] + completion for i, completion in enumerate(batch_completions)]

        # responses: [response]
        responses = batch_completions
